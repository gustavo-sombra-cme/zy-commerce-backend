    ...
    ILogger<HttpAssistantLlmClient> logger) : IAssistantLlmClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!llmOptions.Enabled
            ...
            || string.IsNullOrWhiteSpace(llmOptions.Model)
            || !TryGetEndpoint(llmOptions, out var endpoint)
            || !TryGetApiKey(llmOptions, out var apiKey))
        {
            return null;
        }
        ...
        request.Content = new StringContent(
            JsonSerializer.Serialize(CreateRequestBody(llmOptions.Model, question), JsonOptions),
            Encoding.UTF8,
            "application/json");
    ...
    private const string SystemPrompt = """
        ...
        Allowed kind values: Unsupported, RecentOrders, TotalSpend, ProductsOrdered, OrdersContainingProduct, OrdersAboveAmount, OrdersContainingProductsOverAmount, ProductFrequency, CatalogProductsUnderPrice, CatalogGetProduct.
