...
Architecture project discovery scans all project files in the current checkout except paths beneath an exact `.worktrees` directory segment. This keeps nested Git worktrees from duplicating project names while preserving unapproved-project detection everywhere else.
...
Assistant broad catalog search is implemented as read-only API-layer orchestration. `CatalogSearchProducts` maps natural product discovery questions to `CatalogAssistantSubAgent`, which dispatches the existing Catalog Application `SearchProductsQuery` with `IsActive = true`, page `1`, and page size `10`. Catalog search matches SKU, Name, and nullable Description through the infrastructure-only product search read model. It reuses `AssistantResponseTypes.CatalogProducts`, `AssistantCatalogProductsData`, `AssistantProductCardDto`, and `catalog_search`; no frontend contract, Text-to-SQL internal, MCP, schema, migration, raw SQL exposure, `genericTable`, or admin/write assistant behavior changed.
